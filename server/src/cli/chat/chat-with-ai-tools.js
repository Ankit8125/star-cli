...
async function initConversation(userId, conversationId=null, mode="tool"){
  ...
  if(conversation.messages?.length > 0){
    console.log(chalk.yellow("🗒️Previous messages: \n"))
  ...
  try {
    ...
    if(result.toolResults && result.toolResults.length > 0){
      const toolResultBox = boxen(
        result.toolResults.map(toolResult => `${chalk.green("✅ Tool: ")}${toolResult.toolName}\n${chalk.gray("Result: ")}${JSON.stringify(toolResult.result, null, 2).slice(0, 200)}...`).join("\n\n"), {
          padding: 1,
          margin: 1,
          borderStyle: "round",
          borderColor: "green",
...
async function updateConversationTitle(conversationId, userInput, messageCount){
  // When the very first message is added to a conversation (messageCount === 1), this function sets the conversation's title based on the user's input (truncated to 50 characters).
  if(messageCount === 1){
...
export async function startToolChat(conversationId = null){
  try {
    intro(
      boxen(chalk.bold.cyan("🌟Star AI - Tool Calling Mode"), {
        padding: 1,
        borderStyle: "double",
        borderColor: "cyan"
      })
